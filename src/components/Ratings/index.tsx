...
export function Ratings({ recipeId }: RatingsProps) {
  ...
  const handleSubmit = useCallback(() => {
    addRate({
      ...
      addToast({
        title: "Sucesso",
        type: "success",
        description: "Sua avaliação foi enviada com sucesso, aguarde aprovação, obrigado"
      });
    }).catch(() => {
      addToast({
        title: "Erro",
  ...
  return (
    <Container>
      ...
      <textarea id="commentary" name="commentary" value={commentary} onChange={e => setCommentary(e.target.value)} />
      <button type="button" onClick={handleSubmit}>Enviar</button>
      </AddContainer>

      {waiting.length > 0 && waiting.map(comment => (
        <Waiting>
          <h3>Aguardando aprovação</h3>
          <div>
            <div><Stars rate={+comment.rate} /></div>
      ...
      {approved.length > 0 && approved.map(comment => {
